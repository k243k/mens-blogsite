...
import type { CategorySummary, PostSummary, TagSummary } from "@/content/types";
...
export function SearchPageContent({
  ...
  const filtered = useMemo(() => {
    return posts.filter((post) => {
      if (categorySlug && !post.categories.some((category) => category.slug === categorySlug)) {
        return false;
      }
      ...
      if (normalizedQuery) {
        const haystack = `${post.title} ${post.excerpt} ${post.categories.map((category) => category.name).join(" ")} ${post.tags.map((tag) => tag.name).join(" ")}`.toLowerCase();
  ...
  }, [posts, categorySlug, tagSlug, normalizedQuery]);

  const totalPages = Math.max(Math.ceil(filtered.length / PAGE_SIZE), 1);
  const currentPage = Math.min(Math.max(Number.isFinite(currentPageParam) && currentPageParam > 0 ? Math.floor(currentPageParam) : 1, 1), totalPages);
      ...
      <section className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-6 pb-24 pt-16">
        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            {!paginatedPosts.length ? <p className="text-sm text-foreground/60">条件に合う記事が見つかりませんでした。</p> : null}
          </div>
        </section>
